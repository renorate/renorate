...
export default function ContractorProjectsPage() {
  ...
  const fetchProjects = async (userId: string) => {
    try {
      const response = await fetch(`/api/projects?userId=${userId}&role=CONTRACTOR`)
    ...
    <div className="max-w-7xl mx-auto p-8">
      ...
      {projects.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">You haven't been assigned to any projects yet.</p>
          <Link
            href="/estimate/new"
                ...
                <div className="flex-1">
                  ...
                  {project.homeowner && (
                    <p className="text-sm text-gray-500 mb-3">
                      Client: {project.homeowner.name} ({project.homeowner.email})
                    </p>
                  )}
